...
class MarketEnvironment:
    ...
    def __init__(
        self, randomSeed=0, lqd_time=LIQUIDATION_TIME, num_tr=NUM_N, lambd=LLAMBDA
    ):
    ...
    def reset(
        ...
        self.initial_state = np.array(
            ...
            + [self.timeHorizon / self.num_n, self.shares_remaining / self.total_shares]
        )
    ...
    def step(self, action):
        ...
        if self.transacting and (
            self.timeHorizon == 0 or abs(self.shares_remaining) < self.tolerance
        ):
            self.transacting = False
            info.done = True
            info.implementation_shortfall = (
                self.total_shares * self.startingPrice - self.totalCapture
            )
        ...
        if self.transacting:
            ...
            reward = (abs(self.prevUtility) - abs(currentUtility)) / abs(
                self.prevUtility
            )
    ...
    def temporaryImpact(self, sharesToSell):
        ...
        ti = (self.epsilon * np.sign(sharesToSell)) + (
