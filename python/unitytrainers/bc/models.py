...
class BehavioralCloningModel(LearningModel):
    def __init__(
        ...
        if brain.vector_action_space_type == "discrete":
            ...
            self.sample_action_float = tf.identity(
                self.sample_action_float, name="action"
            )
            self.sample_action = tf.cast(self.sample_action_float, tf.int32)
            self.true_action = tf.placeholder(
                shape=[None], dtype=tf.int32, name="teacher_action"
            )
            ...
            self.true_action = tf.placeholder(
                shape=[None, self.a_size], dtype=tf.float32, name="teacher_action"
            )
