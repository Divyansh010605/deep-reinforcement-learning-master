...
class BehavioralCloningTrainer(Trainer):
    ...
    def __init__(self, sess, env, brain_name, trainer_parameters, training, seed):
        ...
        super(BehavioralCloningTrainer, self).__init__(
        ...
        self.n_sequences = max(
            int(trainer_parameters["batch_size"] / self.sequence_length), 1
        )
                    ...
                    if not stored_info_teacher.local_done[idx]:
                        if self.use_visual_observations:
                            for i, _ in enumerate(
                                stored_info_teacher.visual_observations
                            ):
                                self.training_buffer[agent_id][
                                    "visual_observations%d" % i
                                ].append(
                                    stored_info_teacher.visual_observations[i][idx]
                                )
                        if self.use_vector_observations:
                            self.training_buffer[agent_id][
                                "vector_observations"
                        ...
                        if self.use_recurrent:
                            if stored_info_teacher.memories.shape[1] == 0:
                                stored_info_teacher.memories = np.zeros(
                                    (len(stored_info_teacher.agents), self.m_size)
                                )
    ...
    def process_experiences(self, current_info: AllBrainInfo, next_info: AllBrainInfo):
        ...
        for l in range(len(info_teacher.agents)):
            if (
                info_teacher.local_done[l]
                or len(self.training_buffer[info_teacher.agents[l]]["actions"])
        ...
        for l in range(len(info_student.agents)):
            if info_student.local_done[l]:
                ...
                self.stats["episode_length"].append(self.episode_steps.get(agent_id, 0))
                self.cumulative_rewards[agent_id] = 0
                self.episode_steps[agent_id] = 0
    ...
    def update_model(self):
        ...
        for j in range(
            min(
                len(self.training_buffer.update_buffer["actions"]) // self.n_sequences,
            ...
            if self.is_continuous_action:
                feed_dict[self.model.true_action] = np.array(
                    _buffer["actions"][start:end]
            ...
            if self.use_vector_observations:
                if not self.is_continuous_observation:
                    feed_dict[self.model.vector_in] = np.array(
                        _buffer["vector_observations"][start:end]
            ...
            if self.use_recurrent:
                feed_dict[self.model.memory_in] = np.zeros(
                    [self.n_sequences, self.m_size]
                )
            loss, _ = self.sess.run(
                [self.model.loss, self.model.update], feed_dict=feed_dict
            )
