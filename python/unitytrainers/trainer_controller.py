...
class TrainerController(object):
        ...
        for brain_name in self.trainers.keys():
            if self.trainers[brain_name].graph_scope is not None:
                ...
                if self.trainers[brain_name].parameters["trainer"] == "imitation":
                    ...
                    nodes += [
                        scope + x for x in ["action", "value_estimate", "action_probs"]
                    ]
    ...
    def _save_model(self, sess, saver, steps=0):
        ...
        tf.train.write_graph(
            sess.graph_def, self.model_path, "raw_graph_def.pb", as_text=False
        )
        ...
        for brain_name in self.env.external_brain_names:
            if trainer_parameters_dict[brain_name]["trainer"] == "imitation":
                ...
                raise UnityEnvironmentException(
                    "The trainer config contains an unknown trainer type for brain {}".format(
                        brain_name
                    )
                )
        ...
        try:
            ...
            raise UnityEnvironmentException(
                """Parameter file could not be found here {}.
                                            Will use default Hyper parameters""".format(
                    self.trainer_config_path
                )
            )
        except UnicodeDecodeError:
            raise UnityEnvironmentException(
                "There was an error decoding Trainer Config from this path : {}".format(
                    self.trainer_config_path
                )
            )
