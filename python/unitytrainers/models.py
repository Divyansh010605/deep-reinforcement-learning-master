...
class LearningModel(object):
    ...
    def create_vector_input(self, name="vector_observation"):
        ...
        if self.brain.vector_observation_space_type == "continuous":
            self.vector_in = tf.placeholder(
                shape=[None, self.o_size], dtype=tf.float32, name=name
            )
            if self.normalize:
            ...
            self.vector_in = tf.placeholder(
                shape=[None, 1], dtype=tf.int32, name="vector_observation"
            )
            return self.vector_in
    ...
    @staticmethod
    def create_continuous_observation_encoder(
    ...
    def create_visual_observation_encoder(
        ...
        with tf.variable_scope(scope + "/" + "flat_encoding"):
            hidden_flat = self.create_continuous_observation_encoder(
                hidden, h_size, activation, num_layers, scope, reuse
            )
        return hidden_flat

    @staticmethod
    def create_discrete_observation_encoder(
        observation_input, s_size, h_size, activation, num_layers, scope, reuse
    ):
        """
        ...
        for i in range(num_streams):
            ...
            if brain.vector_observation_space_size > 0:
                if brain.vector_observation_space_type == "continuous":
                    hidden_state = self.create_continuous_observation_encoder(
                    ...
                    hidden_state = self.create_discrete_observation_encoder(
                        vector_observation_input,
                        self.o_size,
                        h_size,
                        activation_fn,
    ...
    def create_dc_actor_critic(self, h_size, num_layers):
        ...
        if self.use_recurrent:
            ...
            self.prev_action = tf.placeholder(
                shape=[None], dtype=tf.int32, name="prev_action"
            )
        ...
        self.entropy = -tf.reduce_sum(
            self.all_probs * tf.log(self.all_probs + 1e-10), axis=1
        )
        self.action_holder = tf.placeholder(shape=[None], dtype=tf.int32)
        self.selected_actions = tf.one_hot(self.action_holder, self.a_size)

        self.all_old_probs = tf.placeholder(
            shape=[None, self.a_size], dtype=tf.float32, name="old_probabilities"
        )

        # We reshape these tensors to [batch x 1] in order to be of the same rank as continuous control probabilities.
        self.probs = tf.expand_dims(
            tf.reduce_sum(self.all_probs * self.selected_actions, axis=1), 1
        )
        self.old_probs = tf.expand_dims(
            tf.reduce_sum(self.all_old_probs * self.selected_actions, axis=1), 1
        )
    ...
    def create_cc_actor_critic(self, h_size, num_layers):
        ...
        a = tf.exp(
            -1 * tf.pow(tf.stop_gradient(self.output_pre) - mu, 2) / (2 * sigma_sq)
        )
