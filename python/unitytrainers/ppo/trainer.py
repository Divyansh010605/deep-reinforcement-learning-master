...
from unitytrainers.trainer import Trainer, UnityTrainerException
...
class PPOTrainer(Trainer):
    ...
    def __init__(self, sess, env, brain_name, trainer_parameters, training, seed):
        ...
        super(PPOTrainer, self).__init__(
            sess, env, brain_name, trainer_parameters, training
        )
        ...
        if self.use_recurrent:
            ...
            if self.m_size == 0:
                raise UnityTrainerException(
                    "The memory size for brain {0} is 0 even though the trainer uses recurrent.".format(
                        brain_name
                    )
                )
            elif self.m_size % 4 != 0:
                raise UnityTrainerException(
                    "The memory size for brain {0} is {1} but it must be divisible by 4.".format(
                        brain_name, self.m_size
                    )
                )
        ...
        self.is_continuous_observation = (
            env.brains[brain_name].vector_observation_space_type == "continuous"
        )
        ...
        if (
            self.is_training
            and self.is_continuous_observation
        ...
        if self.use_recurrent:
            if not self.is_continuous_action:
                feed_dict[self.model.prev_action] = (
                    curr_brain_info.previous_vector_actions.flatten()
                )
        ...
        if self.use_visual_obs:
            for i, _ in enumerate(curr_brain_info.visual_observations):
                feed_dict[self.model.visual_in[i]] = (
                    curr_brain_info.visual_observations[i]
                )
        ...
        if self.use_curiosity:
            ...
            if self.is_continuous_action:
                ...
                feed_dict[self.model.action_holder] = (
                    next_info.previous_vector_actions.flatten()
                )
    ...
    def add_experiences(
        ...
        for agent_id in curr_info.agents:
            ...
            self.training_buffer[agent_id].last_take_action_outputs = (
                take_action_outputs
            )
                    ...
                    if self.use_visual_obs:
                        for i, _ in enumerate(stored_info.visual_observations):
                            ...
                            self.training_buffer[agent_id][
                                "next_visual_obs%d" % i
                            ].append(next_info.visual_observations[i][idx])
                    if self.use_vector_obs:
        ...
        for l in range(len(info.agents)):
            ...
            if (
                ...
                self.training_buffer.append_update_buffer(
                    agent_id, batch_size=None, training_length=self.sequence_length
                )
    ...
    def is_ready_update(self):
        ...
        return size_of_buffer > max(
            int(self.trainer_parameters["buffer_size"] / self.sequence_length), 1
        )
    ...
    def update_model(self):
        ...
        """
        n_sequences = max(
            int(self.trainer_parameters["batch_size"] / self.sequence_length), 1
        )
        ...
        for k in range(num_epoch):
            ...
            for l in range(
                ...
                feed_dict = {
                    ...
                    self.model.mask_input: np.array(
                        buffer["masks"][start:end]
                    ).flatten(),
                    self.model.returns_holder: np.array(
                        buffer["discounted_returns"][start:end]
                    ).flatten(),
                    self.model.old_value: np.array(
                        buffer["value_estimates"][start:end]
                    ).flatten(),
                    self.model.advantage: np.array(
                        buffer["advantages"][start:end]
                    ).reshape([-1, 1]),
                    self.model.all_old_probs: np.array(
                        buffer["action_probs"][start:end]
                    ).reshape([-1, self.brain.vector_action_space_size]),
                }
                if self.is_continuous_action:
                    feed_dict[self.model.output_pre] = np.array(
                        buffer["actions_pre"][start:end]
                    ).reshape([-1, self.brain.vector_action_space_size])
                else:
                    feed_dict[self.model.action_holder] = np.array(
                        buffer["actions"][start:end]
                    ).flatten()
                    if self.use_recurrent:
                        feed_dict[self.model.prev_action] = np.array(
                            buffer["prev_action"][start:end]
                        ).flatten()
                if self.use_vector_obs:
                    if self.is_continuous_observation:
                        ...
                        feed_dict[self.model.vector_in] = np.array(
                            buffer["vector_obs"][start:end]
                        ).reshape([-1, total_observation_length])
                        if self.use_curiosity:
                            feed_dict[self.model.next_vector_in] = np.array(
                                buffer["next_vector_in"][start:end]
                            ).reshape([-1, total_observation_length])
                    else:
                        feed_dict[self.model.vector_in] = np.array(
                            buffer["vector_obs"][start:end]
