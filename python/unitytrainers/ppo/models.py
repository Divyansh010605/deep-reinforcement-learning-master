...
class PPOModel(LearningModel):
    ...
    def create_curiosity_encoders(self):
        ...
        if self.o_size > 0:
            ...
            if self.brain.vector_observation_space_type == "continuous":
                ...
                encoded_next_vector_obs = self.create_continuous_observation_encoder(
                    self.next_vector_in,
                    self.curiosity_enc_size,
                    self.swish,
                    2,
                ...
                self.next_vector_in = tf.placeholder(
                    shape=[None, 1], dtype=tf.int32, name="next_vector_observation"
                )

                encoded_vector_obs = self.create_discrete_observation_encoder(
                    self.vector_in,
                    self.o_size,
                    self.curiosity_enc_size,
                    self.swish,
                ...
                encoded_next_vector_obs = self.create_discrete_observation_encoder(
                    self.next_vector_in,
                    self.o_size,
                    self.curiosity_enc_size,
                    self.swish,
    ...
    def create_forward_model(self, encoded_state, encoded_next_state):
        ...
        pred_next_state = tf.layers.dense(
            hidden,
            self.curiosity_enc_size * (self.v_size + int(self.o_size > 0)),
            activation=None,
        )
