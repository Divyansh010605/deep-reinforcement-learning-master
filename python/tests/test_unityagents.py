...
import numpy as np
import pytest
from unityagents import (BrainInfo, Curriculum, UnityActionException,
                         UnityEnvironment, UnityEnvironmentException)
...
def test_reset(mock_communicator, mock_launcher):
    ...
    assert brain_info["RealFakeBrain"].vector_observations.shape[0] == len(
        brain_info["RealFakeBrain"].agents
    )
    assert (
        brain_info["RealFakeBrain"].vector_observations.shape[1]
...
def test_step(mock_communicator, mock_launcher):
    ...
    brain_info = env.reset()
    brain_info = env.step(
        [0] * brain.vector_action_space_size * len(brain_info["RealFakeBrain"].agents)
    )
    with pytest.raises(UnityActionException):
        env.step([0])
    brain_info = env.step(
        [-1] * brain.vector_action_space_size * len(brain_info["RealFakeBrain"].agents)
    )
    with pytest.raises(UnityActionException):
        env.step(
            [0]
...
def test_curriculum():
    ...
    with mock.patch("json.load") as mock_load:
        with mock.patch(open_name, create=True) as mock_open:
            ...
            with pytest.raises(UnityEnvironmentException):
                Curriculum(
                    "tests/test_unityagents.py", {"param1": 1, "param2": 1, "param3": 1}
                )
